...
class Brain:
    def __init__(self, num_states, num_actions):
        ...
        self.optimizer = optim.Adam(
            self.main_q_network.parameters(), lr=0.0002)
        ...
        for episode in range(NUM_EPISODES):  # 試行数分繰り返す
            ...
            for step in range(MAX_STEPS):  # 1エピソードのループ
                ...
                if done:  # ステップ数が200経過するか、一定角度以上傾くとdoneはtrueになる
                    ...
                    # CartPole 直近10episodeの立てたstep数リストに追加
                    ...
                    if step+1 < 200:
                        ...
                        complete_episodes = complete_episodes + 1  # 連続成功記録をリセット
                    else:
                        reward = torch.FloatTensor([-1.0])  # 立ったまま終了時は報酬1を与える
                        complete_episodes = 0  # 連続記録を更新
                else:
...
for i_episode in range(2):
    ...
    for t in range(200):
        ...
        print(observation)
        action = cartpole_env.agent.get_action(state, episode=1, mode=True)
